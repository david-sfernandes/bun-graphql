...
const getPayload = (ctx: GraphQLContext) => {
  ...
  if (!payload) {
    ...
    const extractedPayload = extractPayload(ctx.request.headers.get("authorization") || "");
    payload = extractedPayload && typeof extractedPayload === "object" ? extractedPayload as JwtPayload : undefined;
  }
...
const isAuthorized = rule()(async (_: unknown, __: unknown, ctx: GraphQLContext) => {
  const payload = getPayload(ctx);
  return !!(payload?.scope && mainRoles.includes(payload.scope));
});

const isManager = rule()(async (_: unknown, __: unknown, ctx: GraphQLContext) => {
  const payload = getPayload(ctx);
...
const isClient = rule()(async (_: unknown, __: unknown, ctx: GraphQLContext) => {
  const payload = getPayload(ctx);
  return payload?.scope === "CLIENT";
});

const isTechnician = rule()(async (_: unknown, __: unknown, ctx: GraphQLContext) => {
  const payload = getPayload(ctx);
